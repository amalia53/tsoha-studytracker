...
def get_teachers_ongoing_courses(user_id):
	sql = "SELECT DISTINCT courses.id, course FROM courses, teachers WHERE teachers.id=:teacher_id AND user_id=:user_id AND courses.id IN (SELECT course_id FROM goals WHERE grade IS NULL AND NOT deleted)"
