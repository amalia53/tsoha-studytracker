...
def get_teachers_ongoing_courses(user_id):
	sql = "SELECT DISTINCT courses.id, course FROM courses WHERE teacher_id=4 AND courses.id IN (SELECT course_id FROM goals WHERE grade IS NULL AND NOT deleted)"
...
def get_stats_table(user_id):
	...
	for course in results:
		...
		counts.append(round(course_results[0][1],1))
